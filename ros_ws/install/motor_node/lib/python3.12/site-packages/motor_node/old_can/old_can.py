...
def get_sdo_obj(self, publisher) -> dict:
    voltage = read_and_log_sdo(self.node, 0x2030, 2)  # Volts
    throttle_mv = read_and_log_sdo(self.node, 0x2013, 1)  # mV
    rpm = read_and_log_sdo(self.node, 0x2001, 2)  # rpm
    current = read_and_log_sdo(self.node, 0x2073, 1)  # Arms
    temperature = read_and_log_sdo(self.node, 0x2040, 2)  # deg C
